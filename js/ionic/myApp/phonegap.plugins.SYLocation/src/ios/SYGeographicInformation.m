...
@implementation SYGeographicInformation

- (instancetype)initWithDict:(NSDictionary *)dict
{
    if (self = [super init]){
        //挨个的拿到字典里面的内容,通过映射关系,写入的指定的属性里面
        [self dicToObject:dict];
        
        if (![self.FormattedAddressLine isEqualToString:self.Name]){
            self.FormattedAddressLine = [NSString stringWithFormat:@"%@%@", [dict[@"FormattedAddressLines"]lastObject], self.Name];
        }
